...
首页搜索的排序逻辑也在 `index.html` 中完成。当前策略是正文命中优先，其次是块自己的显式标题，再给子章节、章节和书名较低权重；如果 `blockTitle` 只是从当前章节或子章节继承而来，不再按“块标题”给高分，只按章节信息加权。这样可以避免“某个章节下的任意定理”因为继承章节名而压过正文真正命中的条目。`dxtips` 会获得额外类型加分，让理解提示更容易靠前，但仍以正文、显式标题和章节相关性为前提。
...
6. 搜索排序优先考虑正文连续命中，再考虑块显式标题、子章节、章节和书名；继承自章节的块标题不会拿到标题高分，避免章节名造成误排。
7. 搜索结果保留 PDF 路径、章节信息和预览信息，方便用户从关键词跳到具体笔记内容。
